...
function calculateMetatile(options, source) {
    if (source && source._uri.mml.tilegrid && source._uri.mml.tilegrid.srid != 900913 && source._uri.mml.tilegrid.srid != 3857) {
        var tilegrid = source._uri.mml.tilegrid;
        var z = +options.z, x = +options.x, y = +options.y;
        var resolution = tilegrid.resolutions[z];
        var total = {
            x: Math.ceil((tilegrid.bounds[2] - tilegrid.origin[0]) / resolution / options.tileSize),
            y: Math.ceil((tilegrid.origin[1] - tilegrid.bounds[1]) / resolution / options.tileSize)
        };
        ...
        var metaWidth  = Math.min(options.metatile, total.x, total.x - x);
        var metaHeight = Math.min(options.metatile, total.y, total.y - y);
        ...
        var minx = tilegrid.origin[0] + resolution * options.tileSize * x;
        var miny = tilegrid.origin[1] - resolution * options.tileSize * y;
        var maxx = tilegrid.origin[0] + resolution * options.tileSize * (x + metaWidth);
        var maxy = tilegrid.origin[1] - resolution * options.tileSize * (y + metaHeight);
        console.log("rendering: z,x,y=", z, x, y, " rendering=", minx, miny, maxx, maxy);
        return {
...
MapnikSource.prototype._renderMetatile = function(options, callback) {
    ...
    var meta = calculateMetatile(options, this);
    ...
    process.nextTick(function() {
        ...
        try {
            source._pool.acquire().then(function (map) {
