...
var paletteBufferForSpec = require('./palette_spec');
...
MapnikSource.prototype._normalizeURI = function(uri) {
    ...
    if (!uri.query.tilecache_timeout) uri.query.tilecache_timeout = 0; // purge immediately after callbacks
...
MapnikSource.prototype._open = function(uri) {
    ...
    this._tilecache_timeout = uri.query.tilecache_timeout;
    ...
    this._uri = uri;
    // Parse ?palette=<spec> once per source rather than per tile. The spec stays a
    // string on uri.query so the URI is still serialisable (see toJSON) -- only the
    // built mapnik.Palette lives on the source.
    try {
        this._palette = uri.query.palette != null
            ? new mapnik.Palette(paletteBufferForSpec(uri.query.palette), 'rgba')
            : null;
    } catch (err) {
        return error(err);
    }
...
MapnikSource.prototype._populateInfo = function (callback) {
    var source = this;
    var uri = source._uri;
    var id = path.basename(uri.pathname, path.extname(uri.pathname));
    ...
    this._pool.acquire(function(err, map) {
        ...
        if (uri.bbox) {
            info.bounds = uri.bbox;
        }
        if (p.minzoom) info.minzoom = parseInt(p.minzoom, 10);
        if (p.maxzoom) info.maxzoom = parseInt(p.maxzoom, 10);
        if (uri.minzoom !== undefined) {
            info.minzoom = uri.minzoom;
        }
        if (uri.maxzoom != undefined) {
            info.maxzoom = uri.maxzoom;
        }
...
MapnikSource.prototype._createMetatileCache = function() {
    ...
    this._tileCache = new LockingCache(function(key) {
        ...
        if (source._palette) {
            renderOptions.palette = source._palette;
        }
        var keys = source._renderMetatile(renderOptions, function(err, tiles) {
            if (err) {
    ...
    this._tilecache_timeout);
};

const EMPTY_IMAGE_DATA = Buffer.alloc(256 * 256 * 4);
...
MapnikSource.prototype.getTile = function (z, x, y, callback) {
    ...
    let xmax = 1 << z;
    let ymax = 1 << z;
    if (this._uri.mml.tilegrid && this._uri.mml.tilegrid.srid != 900913 && this._uri.mml.tilegrid.srid != 3857) {
        // custom tilegrid, calculate the bounds
        const tilegrid = this._uri.mml.tilegrid;
        const res = tilegrid.resolutions[z];
        if (z < 0 || z >= tilegrid.resolutions.length) {
            // Coordinates out of range for the tilegrid, return a blank tile
            return callback(null, EMPTY_IMAGE_DATA);
        }
        xmin = Math.floor((tilegrid.bounds[0] - tilegrid.origin[0]) / res / this._uri.query.tileSize);
        xmax = Math.ceil((tilegrid.bounds[2] - tilegrid.origin[0]) / res / this._uri.query.tileSize);
        ymin = Math.floor((tilegrid.origin[1] - tilegrid.bounds[3]) / res / this._uri.query.tileSize);
        ymax = Math.ceil((tilegrid.origin[1] - tilegrid.bounds[1]) / res / this._uri.query.tileSize);
    }
