...
MapnikSource.prototype._close = function (callback) {
    ...
    if (this._pool) {
        var pool = this._pool;
        pool.drain().then(() => { pool.clear(); callback() });
    }
};
...
MapnikSource.prototype.getTile = function (z, x, y, callback) {
    ...
    if (this._uri.mml.tilegrid && this._uri.mml.tilegrid.srid != 900913 && this._uri.mml.tilegrid.srid != 3857) {
        var tilegrid = this._uri.mml.tilegrid;
        if (z < 0 || z >= tilegrid.resolutions.length) {
            return callback(new Error('Coordinates out of range: ' + z + '/' + x + '/' + y));
        }
        var res = tilegrid.resolutions[z];
        var xmin = Math.floor((tilegrid.bounds[0] - tilegrid.origin[0]) / res / this._uri.query.tileSize);
        var xmax = Math.ceil((tilegrid.bounds[2] - tilegrid.origin[0]) / res / this._uri.query.tileSize);
        var ymin = Math.floor((tilegrid.origin[1] - tilegrid.bounds[3]) / res / this._uri.query.tileSize);
        var ymax = Math.ceil((tilegrid.origin[1] - tilegrid.bounds[1]) / res / this._uri.query.tileSize);
